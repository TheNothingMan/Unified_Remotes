...
--Combined PDF-VLC for practicing
--xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
--@help Previous
actions.previous_document = function ()
	kb.stroke("ctrl", "shift", "tab");
end

--@help Next
actions.next_document = function ()
	kb.stroke("ctrl", "tab");
end
...
actions.overview = function ()
	if (overview_active) then
		...
		libs.server.update({id = "toggle_overview", text = "Overview"});
		kb.stroke("esc");
	else
		overview_active=true;
		libs.server.update({id = "toggle_overview", text = "Esc"});
		kb.stroke("ctrl", "shift", "v");
	end
end
...
--@help set view to double side sliding
actions.view_double_slide = function ()
	kb.stroke("shift", "alt", "d");
end

--@help set view to double not sliding
actions.view_double = function ()
	kb.stroke("alt", "d");
end

--@help set view to single sliding
actions.view_single_slide = function ()
	kb.stroke("shift", "alt", "s");
end

--@help set view to single not sliding
actions.view_single = function ()
	kb.stroke("alt", "s");
end
...
function test()
	...
	if (resp == nil or resp.status ~= 200) then
		libs.device.toast("Could not connect to VLC.\nRefer to VLC Advanced!");
		return false;
	else
		return true;
	end
...
actions.vlc_fullscreen = function ()
	send("fullscreen");
end
