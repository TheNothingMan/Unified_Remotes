...
events.focus = function ()
	...
	mysoc = require("socket").new();
	socket_host = settings.socketHost;
	socket_port = settings.socketPort;
	mysoc:onconnect(function()
		libs.device.toast("socket connected");
		---mysoc:write(b:readstring());
		--mysoc:close();
	end);
	mysoc:onerror(function()
		libs.server.update({
			type = "dialog",
			title = "Socket Connection",
			text = "A connection to VLC socket could not be established." ..
				"We recommend using the latest version of VLC.\n\n" ..
				"1. Make sure VLC is running on your computer.\n\n" ..
				"2. Enable RC interface in VLC settings > Show all settings > Interface > Main interfaces.\n\n" ..
				"3. Enter host and port in VLC settings > Interface > RC > TCP\n\n" ..
				"4. You can also specify a different host or port in the remote settings.\n\n" ..
				"You may have to restart VLC after enabling the RC interface for the changes to take effect.",
			children = {{ type = "button", text = "OK" }}
		});
	end);
	mysoc:ondata(listener);
	mysoc:onclose(function()
		mysoc:connect(socket_host, socket_port);
	end);
	mysoc:connect(socket_host, socket_port);
end
...
--@help Aspect ratio chooser in playback tab
actions.ratio = function ()
	local b = require("buffer").new("utf8");
	b:writestring("vratio\r\n");
	ratios = {};
	local start = false;
	
	mysoc:ondata(function(data)
		print(data);
		b:writestring(data);
		--@help Check if all tracks are sent, then process output and ask the user
		if (string.find(data, "end") ~= nil) then
			local line = b:readline();
			while line ~= nil do
				if (string.find(line, "+") ~= nil) then
					start = true;
				end;
				if (string.find(line, "|") ~= nil) and start then
					local id;
					_, _, id = string.find(line, "- (.+)");
					table.insert(ratios, {type = "item", text=line, id = id});
				end
				line = b:readline();
			end
			mysoc:ondata(listener);
...
actions.ratio_dialog = function (i)
	i = i + 1;
	local action = ratios[i].id;
	print(action);
	mysoc:write("vratio "..action.."\r\n");
end

------------------------------------------------------------------------
-- DVD
------------------------------------------------------------------------
actions.dvd_up = function ()
	kb.stroke("up");
end

actions.dvd_left = function ()
	kb.stroke("left");
end

actions.dvd_right = function ()
	kb.stroke("right");
end

actions.dvd_down = function ()
	kb.stroke("down");
end

actions.dvd_ok = function ()
	kb.stroke("return");
end

actions.dvd_audio = function ()
	local b = require("buffer").new("utf8");
	...
	mysoc:ondata(function(data)
		...
		if (string.find(data, "end") ~= nil) then
			...
			while line ~= nil do
				...
				if (string.find(line, "|") ~= nil) and start then
					table.insert(atracks, {type = "item", text=line, id = string.sub(line, string.find(line, "%-*%d+"))});
				end
				line = b:readline();
			end
			print(atracks);
			...
			libs.server.update({
				type = "dialog",
				ontap = "audio_track_dialog",
				title = "Audio track",
				children = atracks
				--children = {{type = "item", text="test", id="testid"}}
			});
		end
	end);
	mysoc:write(b:readstring());
...
actions.audio_track_dialog = function (i)
	i = i + 1;
	local action = atracks[i].id;
	print(action);
	mysoc:write("atrack "..action.."\r\n");
end

actions.dvd_subtitle = function ()
	local b = require("buffer").new("utf8");
	b:writestring("strack\r\n");
	stracks = {};
	local start = false;
	...
	mysoc:ondata(function(data)
		...
		if (string.find(data, "end") ~= nil) then
			...
			while line ~= nil do
				...
				if (string.find(line, "|") ~= nil) and start then
					print(string.sub(line, string.find(line, "%-*%d+")));
					table.insert(stracks, {type = "item", text=line, id = string.sub(line, string.find(line, "%-*%d+"))});
				end
				line = b:readline();
			end
			mysoc:ondata(listener);
...
actions.sub_track_dialog = function (i)
	i = i + 1;
	local action = stracks[i].id;
	print(action);
	mysoc:write("strack "..action.."\r\n");
end

actions.dvd_menu = function ()
	local b = require("buffer").new("utf8");
	b:writestring("key key-disc-menu\r\n");
	mysoc:write(b:readstring());
end

actions.volume_up_sys = function ()
	kb.press("volumeup");
end

actions.volume_down_sys = function ()
	kb.press("volumedown");
end
...
actions.dvd_chapter = function ()
	...
	mysoc:ondata(function(data)
		print(data);
		b:writestring(data);
		--@help Check if all chapters are sent, then process output and ask the user
		if (string.find(data, "Currently") ~= nil) then
			local line = b:readline();
			while line ~= nil do
				if (string.find(line, "Currently") ~= nil) then
					_, _, current, max = string.find(line, "(%d+)/(%d+)");
					print("Chapter: "..current.."/"..max);
					for i=0,max,1 do
						table.insert(chapters, {type = "item", text=i, id = i});
					end
				end
				line = b:readline();
			end
			...
			libs.server.update({
				type = "dialog",
				ontap = "chapter_dialog",
				title = "Chapters, current: "..current,
				children = chapters
			});
		end
	end);
...
actions.chapter_dialog = function (i)
	i = i + 1;
	local action = chapters[i].id;
	print(action);
	mysoc:write("chapter "..action.."\r\n");
end